...
export const getDumbHandler = (warn) => {
  if (warn) {
    ...
    console.warn('No reliable way to measure text, Texbox will guesstimate.');
  }
  const fCache = {};
  return (text, font) => {
    if (!fCache[font]) {
...
const measure = getCanvasHandler(_doc) || getSvgHandler(_doc) || getDumbHandler(true);

const wCache = {};
export default function get_word_width (token, font, with_shy) {
