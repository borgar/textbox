...
const re_graphemetext = /[\u4E00-\u9FFF]/;
const re_left_symbols = createRegExpHook("'“‘[「【﹝〔<‹«{『（(<《");
const re_right_symbols = createRegExpHook("',，,！!?？”]」】》>’»﹞〕〗〉})）』");

function createRegExpHook (characterSet) {
  characterSet = characterSet.split('');

  return {
    test (chat) {
      return characterSet.indexOf(chat) !== -1;
    }
  };
}
...
export default function parser (text, _trim) {
  ...
  for (let i = 1, l = text.length; i < l; i++) {
    ...
    const has_graphemetext = re_graphemetext.test(last_ch) || re_graphemetext.test(curr_ch); // Chinese text can be wrapped at will.
    let allow_break = curr_is_ws || last_is_ws || has_graphemetext;
    let pre_last_is_ws;
    ...
    // Regarding paired symbols, refer to Microsoft Edge.
    ...
    if (re_right_symbols.test(curr_ch) || re_left_symbols.test(last_ch)) {
      allow_break = false;
    }
